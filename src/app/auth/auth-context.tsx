...
interface AuthContextValue {
  ...
  setHasSeenPreLaunch: () => Promise<void>;
}
...
function mergeUserWithProfile(
  ...
  return {
    ...
    hasSeenPreLaunch:
      profile?.hasSeenPreLaunch ??
      user.hasSeenPreLaunch ?? false,
  };
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signup = useCallback(
    async ({ fullName, email, password, organization }: SignupCredentials) => {
      ...
      try {
        ...
        const user = mergeUserWithProfile(
          {
            ...response.user,
  ...
  const setHasSeenPreLaunch = useCallback(
    async () => {
      if (!session) return;
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [error, isLoading, login, logout, session, setPurposes, setRole, setHasSeenPreLaunch, signup],
  );
